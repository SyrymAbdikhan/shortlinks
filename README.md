...
- **pytest + pytest-asyncio** — test suite (SQLite in-memory, no running DB needed)
...
```
```json
{"code": "aB3xYz", "url": "https://example.com/some/long/path", "created_at": "2024-01-01T00:00:00Z"}
```
...
Tests use an isolated in-memory SQLite database — no running PostgreSQL required.
...
tests/
├── conftest.py          # fixtures (AsyncClient, isolated SQLite db)
└── test_links.py        # 14 tests
```